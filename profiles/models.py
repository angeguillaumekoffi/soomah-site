...
class BaseProfile(models.Model):
    ...
    pays = models.CharField(max_length=25, choices=paysTuple, verbose_name="Pays", blank=True, null=False)
    contact = PhoneNumberField(verbose_name="Téléphone", unique=True, max_length=20,   blank=True, null=True)
...
class Profile(BaseProfile):
    def __str__(self):
        return "{} (${})".format(self.user.name, self.montant)
...
class Subscriber(models.Model):
    user_rec = models.ForeignKey(Profile, verbose_name="Souscripteur", on_delete=models.CASCADE)
