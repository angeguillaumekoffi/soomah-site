...
def create_checkout_session(request):
    #verif_delai()
    ex_price = 'price_1GvkZOGUhhkHEKB5rl9pWG1O'
    if request.method == 'GET':
        domain_url = 'https://soomah.org/utilisateur/me/'
        ...
        try:
            ...
            checkout_session = stripe.checkout.Session.create(
                ...
                mode='subscription',
                line_items=[{ 'price': 'price_1GvkZOGUhhkHEKB5rl9pWG1O', 'quantity': 1}]
            )
        ...
        try:
            if slug:
                ...
                try:
                    ...
                    cap = models.Profile.objects.get(user = usr)
                    if cap.contact == None:
                        profile.delete()
                    else:
                        user = usr
            ...
            test = models.Profile.objects.get(user = user)
            ...
            try:    
                ...
                "Bienvenue {}, vous devez vous abonner à valeur de 55,00 $US pour un mois afin de pouvoir apporter et recevoir de l'aide !  Merci de vous abonner.".format(user.name),
                )
            ...
            recent = models.ApporterAide.objects.filter(emetteur=user.profile, etatDapport=False, activTransact=False).order_by("-dateDeSoumission")[:4]
            ...
            if abonnement != None:
                if not abonnement.statutCompte:
                    messages.warning(request, 
                    "Bienvenue {}, vous devez vous abonner à valeur de 55,00 $US pour un mois afin de pouvoir apporter et recevoir de l'aide !"
                    "  Merci de vous abonner.".format(user.name),
                    )
            return super().get(request, *args, **kwargs)
        except:
            messages.warning(request, "Veuillez terminer votre inscription pour continuer.")
            return redirect("profiles:infosPerso")
...
class VueInfosPerso(LoginRequiredMixin, generic.TemplateView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        try:
            ...
            if "user_form" not in kwargs:
                kwargs["user_form"] = forms.UserForm(instance=user)
            if "profile_form" not in kwargs:
                kwargs["profile_form"] = forms.InfoPersoForm()
    ...
    def post(self, request, *args, **kwargs):
        user = self.request.user
        user_form = forms.UserForm(request.POST, instance=user)
        profile_form = forms.InfoPersoForm(request.POST, request.FILES)
        ...
        try:
            ...
            messages.success(request, "Félicitation {}! vous avez validé toutes les étapes de l'inscription. Vous devez payer votre abonnement mensuel de 55,00 $US pour pouvoir apporter et recevoir de l'aide  !".format(profil.user.name))
            return redirect("profiles:show_self")

        except:
            messages.warning(
                request,
                "Echec de validadation ! Veuillez saisir un pays et un numéro de téléphone compatibles à votre position actuelle. ***Exemple: USA +1 XXXXXXXXXX ***",
            )
            user_form = forms.UserForm(instance=user)
...
class VueConfirmerEnvoi(LoginRequiredMixin, generic.TemplateView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        user = self.request.user
        try:
            occurance= models.ApporterAide.objects.get(id=kwargs["Id"], etatDapport=False)
            ...
            messages.success(request, "Erreur ! élément non trouvé.")
...
class SuccessView(LoginRequiredMixin, generic.TemplateView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        try:    
            ...
            abonnement = None
        sess = kwargs["session_id"]
        kwargs["show_user"] = user
        kwargs["Abonnement"] = abonnement
        messages.success(request, "Félicitations ! Votre abonnement a réussi. Vous êtes desormais membre actif de la communauté SOOMAH. Vous pouvez à présent apporter de l'aide et en recevoir.  Session = {}".format(sess))
        return super().get(request, *args, **kwargs)


class CancelledView(LoginRequiredMixin, generic.TemplateView):
    template_name = 'profiles/canceled.html'
    http_method_names = ["get"]
    ...
    def get(self, request, *args, **kwargs):
        ...
        kwargs["show_user"] = user
        kwargs["Abonnement"] = abonnement
        messages.warning(request, "Votre abonnement a échoué! Veuillez vous assurer que votre carte de crédit est bien valide et débitable puis reéssayez ! SOOMAH vous fait confiance.")
        return super().get(request, *args, **kwargs)
