...
class LoginView(bracesviews.AnonymousRequiredMixin, authviews.LoginView):
    ...
    form_valid_message = "Vous avez passé la première étape. Veuillez completer vos informations personnelles pour accéder à votre compte"

    def form_valid(self, form):
        redirct = super().form_valid(form)
...
class SignUpView(generic.TemplateView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            form.is_valid()
            form.save()
            messages.success(request, "Vous avez fini la première étape. Connectez-vous maintenant et completez vos informations personnelles pour terminer votre inscription !")
            return redirect("comptes:login")

        except Exception as ex:
            form = forms.SignupForm()
            messages.warning(request, "Erreur! veuillez reprendre. exception : {}".format(ex))
